...
def expand_image(image, left, top, right, bottom):
    """Expand image with padding on specified sides."""
    h, w = image.shape[:2]
    new_h = h + top + bottom
    new_w = w + left + right
    
    # Create expanded image with zeros
    if len(image.shape) == 3:
        expanded = np.zeros((new_h, new_w, image.shape[2]), dtype=image.dtype)
        expanded[top:top+h, left:left+w, :] = image
        # Create mask (1 where original image is, 0 where padding is)
        mask = np.zeros((new_h, new_w), dtype=np.uint8)
        mask[top:top+h, left:left+w] = 255
    else:
        expanded = np.zeros((new_h, new_w), dtype=image.dtype)
        expanded[top:top+h, left:left+w] = image
        mask = np.zeros((new_h, new_w), dtype=np.uint8)
        mask[top:top+h, left:left+w] = 255
