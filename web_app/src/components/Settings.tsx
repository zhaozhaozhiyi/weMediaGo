    ...
    if (showModelSwitching) {
      ...
      if (shouldSwitchModel) {
        newModelSwitchingTexts.push(
          `正在将模型从 ${settings.model.name} 切换为 ${model.name}`
        )
      }
      if (shouldSwitchRemoveBGModel) {
        newModelSwitchingTexts.push(
          `正在将 RemoveBG 模型从 ${serverConfig?.removeBGModel} 切换为 ${values.removeBGModel}`
        )
      }
      if (shouldSwitchRealesrganModel) {
        newModelSwitchingTexts.push(
          `正在将 RealESRGAN 模型从 ${serverConfig?.realesrganModel} 切换为 ${values.realesrganModel}`
        )
      }
      if (shouldSwitchInteractiveModel) {
        newModelSwitchingTexts.push(
          `正在将 ${PluginName.InteractiveSeg} 模型从 ${serverConfig?.interactiveSegModel} 切换为 ${values.interactiveSegModel}`
        )
      }
      ...
      if (shouldSwitchRemoveBGModel) {
        try {
          ...
          toast({
            variant: "destructive",
            title: `切换 RemoveBG 模型为 ${values.removeBGModel} 失败：${error}`,
          })
        }
      }
      ...
      if (shouldSwitchRealesrganModel) {
        try {
          ...
          toast({
            variant: "destructive",
            title: `切换 RealESRGAN 模型为 ${values.realesrganModel} 失败：${error}`,
          })
        }
      }
      ...
      if (shouldSwitchInteractiveModel) {
        try {
          ...
          toast({
            variant: "destructive",
            title: `切换 ${PluginName.InteractiveSeg} 模型为 ${values.interactiveSegModel} 失败：${error}`,
          })
        }
      }
      ...
      <div className="flex flex-col gap-4 w-[510px]">
        ...
        <div className="space-y-4  rounded-md">
          ...
          <Tabs defaultValue={defaultTab}>
            <TabsList>
              <TabsTrigger value={MODEL_TYPE_INPAINT}>修复模型</TabsTrigger>
      ...
      <div className="space-y-4 w-[510px]">
          ...
          render={({ field }) => (
            <FormItem className="flex items-center justify-between">
              <div className="space-y-0.5">
                <FormLabel>启用手动修复</FormLabel>
                <FormDescription>
                  对于擦除模型，画完遮罩后需要点击按钮手动触发修复。
                </FormDescription>
              </div>
              <FormControl>
                <Switch
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem className="flex items-center justify-between">
              <div className="space-y-0.5">
                <FormLabel>启用自动提取提示词</FormLabel>
                <FormDescription>
                  自动从图片元数据中提取提示词/反向提示词。
                </FormDescription>
              </div>
              <FormControl>
                <Switch
      ...
      <div className="space-y-4 w-[510px]">
        <FormField
          ...
          render={({ field }) => (
            <FormItem className="flex items-center justify-between">
              <div className="space-y-0.5">
                <FormLabel>去除背景</FormLabel>
                <FormDescription>用于去除背景的模型</FormDescription>
              </div>
              <Select
              ...
              >
                <FormControl>
                  <SelectTrigger className="w-auto">
                    <SelectValue placeholder="选择 RemoveBG 模型" />
                  </SelectTrigger>
                </FormControl>
          ...
          render={({ field }) => (
            <FormItem className="flex items-center justify-between">
              <div className="space-y-0.5">
                ...
                <FormDescription>超分辨率模型（RealESRGAN）</FormDescription>
              </div>
              <Select
                onValueChange={field.onChange}
                defaultValue={field.value}
              ...
              >
                <FormControl>
                  <SelectTrigger className="w-auto">
                    <SelectValue placeholder="选择 RealESRGAN 模型" />
                  </SelectTrigger>
                </FormControl>
                <SelectContent align="end">
                  <SelectGroup>
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem className="flex items-center justify-between">
              <div className="space-y-0.5">
                <FormLabel>交互式分割</FormLabel>
                <FormDescription>交互式分割模型</FormDescription>
              </div>
              <Select
                onValueChange={field.onChange}
                defaultValue={field.value}
              ...
              >
                <FormControl>
                  <SelectTrigger className="w-auto">
                    <SelectValue placeholder="选择交互式分割模型" />
                  </SelectTrigger>
                </FormControl>
                <SelectContent align="end">
                  <SelectGroup>
